...
def load_data(train_path: str = 'churn-bigml-80.csv',
              test_path: str = 'churn-bigml-20.csv') -> Tuple[pd.DataFrame, pd.DataFrame]:
    """
...
def detect_normality(df: pd.DataFrame, 
                     columns: List[str], 
    ...
    print(f"✓ Normality test: {len(normal_columns)} normal, {len(non_normal_columns)} non-normal")
...
def remove_outliers_zscore(df: pd.DataFrame, 
    ...
    print(f"✓ Z-score outlier removal: {removed_count} rows removed ({removed_count/original_shape*100:.2f}%)")
...
def remove_outliers_iqr(df: pd.DataFrame, 
    ...
    removed_count = original_shape - df_cleaned.shape[0]
    print(f"✓ IQR outlier removal: {removed_count} rows removed ({removed_count/original_shape*100:.2f}%)")
...
def engineer_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if 'Customer service calls' in df_eng.columns and 'Account length' in df_eng.columns:
        df_eng['CScalls Rate'] = df_eng['Customer service calls'] / df_eng['Account length'].replace(0, 1)
...
def predict_churn(
    customer_data: Union[pd.DataFrame, Dict],
    ...
    for col, encoder_key in [('State', 'state_encoder'), ('Area code', 'area_code_encoder')]:
    ...
    correlated = ['Total day minutes', 'Total eve minutes', 'Total night minutes', 
...
def prepare_data(
    ...
    train_clean, _ = remove_outliers_zscore(train_enc, normality_results['normal_columns'])
    train_clean, _ = remove_outliers_iqr(train_clean, normality_results['non_normal_columns'])
